...
/*if (!function_exists('process_hmac_auth')) {
    ...
    function process_hmac_auth($request)
    {
        ...
        // 1. CONVERT DATE TO UNIX TIMESTAMP
        $request_date = date(DATE_RFC1123);
        dd($request_date, strtotime('Thu, 14 Dec 2021 18:36:57 +0100'), $json_payload);
        $request_auth = $request->header('Authorization');
        $request_date = $request->header('TOPALAT-DATETIME');
//        dd($request_date, $request_auth);
//        $user_secret = Config::get('constants.baxi.user_secret');
        $user_secret = 'secret';
        $timestamp = strtotime($request_date);
        $timestamp = '';
...
//        dd("sha1", $json_payload, $encoded_signed_string, $user_secret, $raw_output = TRUE);;
        $show = hash_hmac('SHA256', 'HappyWorld', 'utf8_encode(chr(63))', false);
//        dd($show, chr(63));
